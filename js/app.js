...
(function () {
  ...
  $("#logoutBtn").addEventListener("click", () => {
    ...
    applyAuthState();
    renderAll();
    toast("Disconnesso. Modifica disabilitata.");
  });
  $("#loginForm").addEventListener("submit", (e) => {
    ...
    if (pass === ADMIN_PASSWORD) {
      ...
      $("#adminPass").value = "";
      toast("Modifica sbloccata. Ora puoi gestire i contenuti.");
    } else {
  ...
  function renderAll() {
    applyAuthState();
    renderHobbies();
    renderExp("pcto", "#pctoList", $("#pctoSearch").value);
    renderExp("civic", "#civicList", $("#civicSearch").value);
    renderPortfolio();
  }

  /* ---------- 6. Search & filter ---------- */
  $("#pctoSearch").addEventListener("input", (e) => renderExp("pcto", "#pctoList", e.target.value));
  $("#civicSearch").addEventListener("input", (e) => renderExp("civic", "#civicList", e.target.value));
  $("#portfolioFilters").addEventListener("click", (e) => {
    const btn = e.target.closest("[data-filter]");
    if (!btn) return;
    portfolioFilter = btn.dataset.filter;
    $$("#portfolioFilters .pill").forEach((p) => p.classList.toggle("active", p === btn));
    renderPortfolio();
  });
  ...
  const SCHEMAS = {
    ...
    portfolio: { label: "Portfolio Item", fields: ["title", "description"], single: true, multi: false, pdf: false, category: true },
    pcto:      { label: "PCTO Activity", fields: ["title", "org", "date", "description"], single: false, multi: true, pdf: true },
    civic:     { label: "Civic Project", fields: ["title", "org", "date", "description"], single: false, multi: true, pdf: true },
  };
  ...
  // Download CV (generates a simple text resume on the fly)
  $("#downloadCv").addEventListener("click", (e) => {
    e.preventDefault();
    const cv = `ALEX RIVERA — STUDENT CV
========================
Email: alex.rivera@example.com
Location: Springfield, USA

EDUCATION
- Lincoln High School (2023–Present) — Scientific track

SKILLS
- HTML & CSS, JavaScript, Python, UI/UX Design, Public Speaking

PCTO EXPERIENCES
${data.pcto.map((p) => `- ${p.title} @ ${p.org} (${p.date})`).join("\n")}

CIVIC EDUCATION
${data.civic.map((c) => `- ${c.title} @ ${c.org} (${c.date})`).join("\n")}

PORTFOLIO HIGHLIGHTS
${data.portfolio.map((f) => `- [${f.category}] ${f.title}`).join("\n")}
`;
    const blob = new Blob([cv], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "Alex-Rivera-CV.txt";
    a.click();
